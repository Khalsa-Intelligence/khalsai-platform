...
async def on_fetch(request, env):
    url_str = request.url
    # Parse path cleanly from full URL
    ...
    if method == "POST" and path == "/api/submit_score":
        try:
            ...
            name = data.get("name")
            if not name:
                return Response.new(json.dumps({"status": "error", "message": "Invalid data"}), status=400, headers={"Content-Type": "application/json"})
            ...
            return Response.new(json.dumps({"status": "success", "message": "Score saved to D1 Database!"}), headers={"Content-Type": "application/json"})
        except Exception as e:
            return Response.new(json.dumps({"status": "error", "message": str(e)}), status=500, headers={"Content-Type": "application/json"})
    ...
    if method == "POST" and path == "/api/delete_score":
        try:
            ...
            if data.get("password") != "6789":
                return Response.new(json.dumps({"status": "error", "message": "Incorrect password."}), status=403, headers={"Content-Type": "application/json"})

            target_id = data.get("id")
            query = "DELETE FROM leaderboard WHERE id = ?"
            await env.DB.prepare(query).bind(target_id).run()

            return Response.new(json.dumps({"status": "success", "message": "Entry deleted successfully."}), headers={"Content-Type": "application/json"})
        except Exception as e:
            return Response.new(json.dumps({"status": "error", "message": str(e)}), status=500, headers={"Content-Type": "application/json"})
    ...
    if method == "POST" and path == "/api/reset_leaderboard":
        try:
            ...
            return Response.new(json.dumps({"status": "success", "message": "Leaderboard reset successfully."}), headers={"Content-Type": "application/json"})
        except Exception as e:
            return Response.new(json.dumps({"status": "error", "message": str(e)}), status=500, headers={"Content-Type": "application/json"})
    ...
    if method == "GET" and path == "/leaderboard":
        try:
            # Query D1 database ordered by Level desc, Score desc
            ...
            return Response.new(f"Error loading leaderboard: {str(e)}", status=500)
